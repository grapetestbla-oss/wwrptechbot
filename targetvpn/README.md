...
## Оплата картой (Telegram Payments)

Токен провайдера выдаёт @BotFather: **Payments** → выбрать банк (например
Smart Glocal) → бот получает строку вида `1877036958:TEST:…`. Метка `:TEST:`
означает тестовый режим: карта списаний не делает, подходят тестовые карты
провайдера (для Smart Glocal — `4242 4242 4242 4242`, любая будущая дата, CVC 123).

Токен кладётся в `.env` как `PAYMENT_PROVIDER_TOKEN`, после чего в мини-аппе
появляется способ «💳 Банковской картой». Счёт открывается прямо в Telegram,
подтверждение приходит боту тем же путём, что и для звёзд.

Передать токен при обновлении:

```bash
TVPN_PROVIDER_TOKEN=1877036958:TEST:… bash deploy/bootstrap.sh
```

Ограничение платформы: у Telegram есть минимальная сумма счёта (для рублей
около 60 ₽), поэтому отвязка устройства за 50 ₽ картой не пройдёт — для неё
остаются звёзды, крипта, LZT и баланс.
