...
Переменные при обновлении передавать не нужно: домен и токен берутся из
`/opt/targetvpn/.env`. Клон потерялся — склонируйте заново командой из раздела
«Установка», `.env` останется нетронутым.

Кнопка «Скачать» отдаёт старую сборку — значит ссылка когда-то была задана
вручную, а своя запись в базе перебивает значение по умолчанию. Сбросить:

```bash
/opt/targetvpn/.venv/bin/python /opt/targetvpn/scripts/reset_downloads.py
/opt/targetvpn/.venv/bin/python /opt/targetvpn/scripts/reset_downloads.py --apply
```

Без `--apply` скрипт только показывает, что изменится. После сброса ссылки
снова ведут на последний релиз клиента на GitHub.

Если клиенты подключаются и сразу отваливаются, после обновления
пересинхронизируйте локацию:

```bash
cd /opt/targetvpn && .venv/bin/python scripts/resync_node.py
```

То же самое делает кнопка «Пересинхронизировать с ядром» в админке.
...
```bash
cd /opt/targetvpn-src/targetvpn && bash deploy/test_node.sh
```

Скрипт берёт ключ владельца из базы, сверяет `pbk` из ключа с публичным ключом
в конфигурации ноды, поднимает тестовый клиент Xray прямо на сервере и пробует
через него выйти в интернет. Ответы читаются так:

- **«Ключи не совпадают»** — клиент физически не пройдёт рукопожатие Reality.
  Перевыпустите инбаунд: `bash deploy/install_node.sh` (существующий конфиг
  сохраняется, поэтому сначала удалите `/var/lib/marzban/xray_config.json`).
- **«Туннель работает»** — нода исправна, проблема на стороне клиента.
- **«Через туннель ничего не прошло»** — смотрите логи ядра, они печатаются
  подсказкой в конце.
